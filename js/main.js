...
function initSmoothScroll() {
  document
    .querySelectorAll('a[href^="#"], [onclick*="scrollToSection"]')
    .forEach((link) => {
      if (
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // ❌ Erreur serveur
        showFormError("Une erreur est survenue. Réessayez.");
        submitBtn.disabled = false;
        submitText.textContent = "Envoyer le message";
        submitIcon.classList.remove("hidden");
        loader.classList.add("hidden");
      ...
      // ❌ Erreur réseau
      showFormError("Vérifiez votre connexion internet.");
      submitBtn.disabled = false;
      submitText.textContent = "Envoyer le message";
      submitIcon.classList.remove("hidden");
      loader.classList.add("hidden");
...
window.addEventListener(
  ...
  () => {
    ...
    if (orb2)
      orb2.style.transform = `translate(-${scrollY * 0.03}px, -${scrollY * 0.06}px)`;
  },
  { passive: true },
);
...
console.log(
  "%cYannick Zomahoun — Portfolio",
  "color: #FF2D20; font-weight: bold; font-size: 14px;",
);
console.log(
  "%cDéveloppeur Web Laravel & Enseignant | Bénin 🇧🇯",
  "color: #a0a0b8; font-size: 12px;",
);
console.log(
  "%c📧 yannickzomahoun75@gmail.com",
